...
`dom.js`、`state.js` 和 `roster-model.js` 是基础模块。`roster-store.js` 只依赖模型与可选的 `roster-storage.js`；渲染器和交互模块只通过 Store 读写业务数据，不直接修改业务数组。`navigation.js`、`gestures.js`、`drawer.js` 与 `toast.js` 只依赖 UI 基础模块（`gestures.js` 还使用导航的渲染接口）。`student-font-size.js` 依赖 UI 基础模块和 Toast 接口；`seat-canvas.js` 依赖固定 DOM、`seat-geometry.js` 与 Store 操作接口，并独立保存非持久化的画布 transform。`main.js` 只负责创建 Store、初始化模块和注入接口，避免循环依赖。

...
| 位置 | 职责 |
| --- | --- |
| `index.html` | 静态语义结构、三页内容、导航、抽屉与 Toast 文案。修改页面文案或卡片内容请编辑此文件。 |
...
| `styles/controls.css` | 底部导航、菜单按钮、抽屉、遮罩和 Toast。 |
...
`dom.js` 和 `state.js` 是基础模块。`navigation.js`、`gestures.js`、`drawer.js` 与 `toast.js` 只依赖基础模块（`gestures.js` 还使用导航的渲染接口）。`main.js` 负责导入并初始化全部功能；它将 `openDrawer` 注入手势模块，避免循环依赖。

修改导航请编辑 `scripts/navigation.js`，修改手势请编辑 `scripts/gestures.js`，修改抽屉请编辑 `scripts/drawer.js`。视觉修改应按对应样式模块定位。

...
表格交换模块将 Store 快照转换为五个固定工作表：`workbook-transfer.js` 负责版本识别、五表生成、隐藏编号读取、数据校验和整库替换前的数据准备；`xlsx-workbook.js` 负责 Office Open XML、ZIP、隐藏行列、冻结窗格、筛选、合并表头、批注、样式和输入限制。版本 1 的十二工作表读取函数与旧 CSV 读取路径独立保留，导出只走版本 2。

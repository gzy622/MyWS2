...
XLSX 导入导出由 `src/scripts/workbook-transfer.js` 实现，底层 Office Open XML/ZIP 处理位于 `src/scripts/xlsx-workbook.js`，文件读写与 JSON 备份共用 `src/scripts/text-file-transfer.js`。新导出格式为版本 3，固定按顺序生成「座位表」「作业登记」「班干安排」「值日安排」「课程表」「考试成绩」六个工作表；版本 2 的五表、版本 1 的十二表和旧 CSV 仍从「导入表格」读取。座位表 A1:M10 是唯一可见区，包含 8×13 行优先矩阵和下方讲台；姓名竖排，打印区域固定为竖向 A4 单页，名单和稳定编号隐藏保存。作业 A/B 列为 8/14，C 列起居中；课程表节次/星期列为 10/12；考试成绩 A/B/成绩列为 8/14/10。班干和值日成员格只显示以「；」分隔的姓名，稳定编号和原显示快照隐藏保存；检查失败会拒绝整份文件并提示 `工作表!单元格：说明`。

表格快速检查应覆盖：v3 六表顺序、座位表仅显示 A1:M10、8×13 每格和空位、竖排姓名、讲台、A4 纵向单页打印参数、隐藏交换行列、各表列宽、作业项居中、班干和值日单格纯姓名、空成员、未改动重名恢复、编辑后重名报错、课程表十个节次、考试/科目两层表头、缺少行列、重复座位、多个当前作业、无效成绩，以及版本 2、版本 1、旧 CSV 和 JSON 备份互不影响。可以使用常用表格软件打开生成文件并重新保存，再交回 `parseRosterWorkbook` 检查隐藏编号、空白矩阵、打印设置和列级默认样式仍可读取。

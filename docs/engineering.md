...
- `navigation.js` 统一将页面状态渲染到轨道、顶栏、导航、分段、子视图和指示点；落位完成后若主页面或子视图相对上次通知有变化，再通知可见业务视图协调器；跟手拖动期间不通知。成绩表等仅清除跟手态的 `renderNavigation()` 不触发重建。
...
- 外壳瞬时态：`.is-sheet-gesturing`、`.is-page-gesturing`（跟手/落位时关闭顶栏 backdrop-filter）；设置页横滑跟手：`.menu-drawer.is-dragging`（跟手期间取消 transition）、`.app.is-drawer-gesturing`（跟手时关闭设置页/学生名单头部 backdrop-filter）；全屏左滑面遮罩：`.fullscreen-scrim`、`.drawer-scrim`、`.roster-editor-scrim`（透明度与面板位移 1:1 跟手，`.is-dragging` 期间切换为 `--duration-scrim-track` 追赶过渡）；
...
- 新导出文件名形如 `teacher-workbench-data-YYYYMMDD-HHmmss.xlsx`，格式版本为 `WORKBOOK_FORMAT_VERSION = 3`。导入按固定工作表路由：版本 3 的六表、版本 2 的五表、版本 1 的十二表和旧 CSV 分别走独立解析器；JSON 备份继续由独立入口保留。
- 新 XLSX 恰好生成「座位表」「作业登记」「班干安排」「值日安排」「课程表」「考试成绩」六个固定工作表，顺序不可变。用户增加的个人工作表可保留并在导入时忽略；六个固定工作表不可删除或改名。
- 「座位表」可见区域仅为 A1:M10：A1:M1 显示「座位表 · 讲台在下方」，A2:M9 是完整 8×13 主矩阵，按 `seatIndex = (座位行 - 1) × 13 + (座位列 - 1)` 行优先映射，空位保留为空、姓名以竖排样式显示；A10:M10 合并显示「讲台」。第 11 行起的名单和 N:AP 的元数据、矩阵位置 ID、稳定学生编号全部隐藏，仅用于导入恢复。打印区域固定为 A1:M10，纸张为竖向 A4，单页缩放，窄页边距；学生行顺序和座位限制保持不变。
- 「作业登记」隐藏第一行保存作业编号，第二行 B2 显示「当前作业」并保存唯一标记，第三行 B3 显示「学生姓名」、C3 起保存作业名称，第四行开始为学生行；每项作业只有一列，空白表示未交，`✓` 表示已交未计分，0～100 表示已交且有分数，最多一位小数。A、B 列宽分别为 8、14；C 列起的作业项列（标题、当前标记、`✓` 和数字分数，含空白默认样式）全部水平居中。
- 「班干安排」和「值日安排」各自一行一个实体；同一行全部成员只写入一个「成员」单元格，不再拆成成员列。可见成员序列只显示姓名，使用「；」分隔；姓名中的反斜杠和分隔符以反斜杠转义。实体编号、成员编号序列和原始显示快照保存在隐藏区域：未改成员格时按隐藏编号恢复重名成员，改动后按姓名解析，重名无法唯一识别时拒绝导入。
- 「课程表」可见区域为节次名称和星期一至星期五，列宽分别为 10、12；隐藏列保存节次编号，固定十个节次。「考试成绩」隐藏第一行保存考试/科目编号，第二行显示考试名称并合并同一考试的表头，第三行显示科目名称，隐藏第一列保存学生编号；A、B 和各成绩列宽分别为 8、14、10，后续考试中的同科目标题跟随第一场考试。
- 标题行、学生姓名列和可修改区域使用分组样式；座位矩阵使用窄列、竖排姓名和较高行高，讲台保持克制居中；学生姓名列使用浅灰底，当前作业表头使用淡蓝底，作业项列全部居中，考试成绩数字右对齐，考试分组使用细分隔线。标题行或学生姓名列按工作表设置冻结；成员单元格使用格式说明批注，成绩单元格带输入限制与说明批注。
- 隐藏行列同时保存格式版本、数据版本、各类数量和内部编号。导入要求座位名单行、作业列、班干行、值日行、十个节次、考试列和科目列保持完整，以避免误删导致数据减少。
- 导入先识别 v3/v2/v1 路由，再检查固定工作表、标题、隐藏编号、数量、重复、座位、人员引用、作业标记、成绩范围和完整矩阵。错误使用「工作表!单元格：说明」格式，例如 `作业登记!D8：请输入 ✓ 或 0～100 的分数`；检查全部通过并经确认后才替换业务数据和重置座位画布。
- XLSX 使用浏览器原生 ZIP/Deflate 与 Office Open XML 读写，不增加 Web 运行时依赖；支持隐藏行列、合并表头、批注、筛选、冻结窗格、输入限制、打印区域、页边距和页面设置。输入文件上限 5 MB，解压后的 XML 总量上限 24 MB；JSON 与旧 CSV 输入仍使用 1 MB 上限。

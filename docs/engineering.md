...
- 「座位表」A1:M1 显示「讲台方向 ↓（讲台在下方）」；A2:M9 是完整 8×13 主矩阵，按 `seatIndex = (座位行 - 1) × 13 + (座位列 - 1)` 行优先映射，空位保留为空；A10:M10 合并显示「讲台」。矩阵是应用座位的复刻显示区，A12:D12 下方名单区用于编辑姓名、首字母、座位行和座位列；O:AB 保存格式/数量元数据，AC:AO 保存矩阵位置 ID，AP 保存每行稳定学生编号。学生行顺序决定应用中的名单顺序，座位行限定 1～8，座位列限定 1～13。
- 「作业登记」隐藏第一行保存作业编号，第二行 B2 显示「当前作业」并保存唯一标记，第三行 B3 显示「学生姓名」、C3 起保存作业名称，第四行开始为学生行；每项作业只有一列，空白表示未交，`✓` 表示已交未计分，0～100 表示已交且有分数，最多一位小数。C 列起的作业项列（标题、当前标记、`✓` 和数字分数，含空白默认样式）全部水平居中。
- 「班干安排」和「值日安排」各自一行一个实体；同一行全部成员只写入一个「成员」单元格，不再拆成成员列。成员序列格式为 `姓名（编号 n）`，使用「；」分隔；姓名中的反斜杠、分隔符和全角括号以反斜杠转义。编号是稳定学生 ID，空单元格表示无成员；实体编号和成员编号序列保存在隐藏区域。导入严格解析格式，检查未知、缺失和同一行重复成员，并以准确单元格报错。
- 「课程表」可见区域为节次名称和星期一至星期五，隐藏列保存节次编号，固定十个节次；「考试成绩」隐藏第一行保存考试/科目编号，第二行显示考试名称并合并同一考试的表头，第三行显示科目名称，隐藏第一列保存学生编号；后续考试中的同科目标题跟随第一场考试。
- 标题行、学生姓名列和可修改区域使用分组样式；座位矩阵和讲台使用克制的居中网格，学生姓名列使用浅灰底，当前作业表头使用淡蓝底，作业项列全部居中，考试成绩数字右对齐，考试分组使用细分隔线。标题行或学生姓名列按工作表设置冻结，矩阵表格带筛选；成员单元格使用格式说明批注，成绩单元格带输入限制与说明批注。

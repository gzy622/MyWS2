...
`.\tools\verify-web.ps1`（Edge CDP 自动验收）**不是**每次小改的默认收尾项。

| 时机 | 是否执行 |
| --- | --- |
| 日常小改（单点手势/样式/文案等），且已用真机或本机手动复验相关路径 | 可跳过 |
| 大型改动提交前、合并前，或触及多页/成绩表横拖/幽灵点击/Sheet 合成层等验收脚本覆盖面 | 必须全部通过 |

小改跳过时，仍应完成 §10.1 中除 `verify-web` 外的适用项，以及与改动相称的真机或手动检查。

### 10.1 静态与单元检查

- [ ] `node --check` 检查全部 `src/scripts/*.js`、`lan-server.js` 和 `tools/content-id.cjs`。
- [ ] `node --test tests/*.test.mjs` 全部通过。
- [ ] 按 §10.0：需要时执行 `.\tools\verify-web.ps1` 并全部通过。

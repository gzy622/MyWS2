...
export function initStudentInteractions({ store, showToast, openStudentRecord }) {
  ...
  elements.seatStage.addEventListener('click', (event) => { const card = event.target.closest('.seat-card'); if (!card || performance.now() < suppressClickUntil) return; const studentId = Number(card.dataset.studentId); if (!store.toggleCompletion(studentId)) return; showToast(card.getAttribute('aria-pressed') === 'true' ? '已取消完成' : '已标记完成'); });

...
export function initAssignments({ store, showToast }) {
  ...
  function close() { layer.classList.remove('show'); setActiveOverlay(null); returnFocus?.focus({ preventScroll: true }); }
  ...
  elements.topbarTitle.addEventListener('click', () => { if (state.currentPage !== 1) return; returnFocus = elements.topbarTitle; render(); layer.classList.add('show'); setActiveOverlay('assignments'); layer.querySelector('.assignment-select')?.focus(); });

...
- 除非用户在当前任务明确要求，智能体不得启动浏览器自动验收或浏览器调试，也不得执行 Android 链路测试。前者包括 `tools/verify-web.ps1`、Edge/CDP 和为浏览器调试启动的服务；后者包括 Android 同步、构建、部署、真机检查和 adb 调试。其余情况只执行与改动直接相关的快速检查。

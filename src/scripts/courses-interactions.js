...
export function initCoursesInteractions({ store, showToast, viewport, closeOthers, confirm }) {
  ...
  elements.app.append(slotLayer, periodLayer, subjectLayer, gradeLayer);
  ...
  function openGrade(studentId, subjectId, trigger) {
    ...
    gradeTarget = { studentId, subjectId };
  ...
  function saveGrade() {
    ...
    const result = store.setCourseGrade(gradeTarget.studentId, gradeTarget.subjectId, gradeInput.value);
    if (result !== 'saved') {
  ...
  bindImmediateAction(gradeLayer.querySelector('[data-action="clear"]'), () => {
    if (!gradeTarget) return;
    const ok = store.clearCourseGrade(gradeTarget.studentId, gradeTarget.subjectId);

...
export function initStudentInteractions({ store, showToast, openStudentRecord }) {
  ...
  elements.studentGrid.addEventListener('pointerdown', (event) => {
    ...
    press.timer = setTimeout(() => { if (!presses.has(event.pointerId)) return; clearPress(event.pointerId); suppressClickUntil = performance.now() + CLICK_SUPPRESSION_MS; haptic(Haptic.medium); openStudentRecord(Number(card.dataset.studentId), card); }, LONG_PRESS_MS);
